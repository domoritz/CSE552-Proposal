...
We developed a visualization system providing user intuitive interfaces for understanding and exploration of query execution in distributed database system. Our system collects profiling logs of query execution in distributed workers then aggregates and then shows query processing at three granularities to enable inspection at the resolution that best reflects problems or insights. This would allow users to be able to efficiently debug and improve queries, and could also allow developers to improve query processing. An example of our visualization can be seen in Figure~\ref{fig:fig1}.
...
In particular, our visualization system is built on top of Myria, a distributed big data management system currently being developed in the UW CSE database group. Myria aims towards building a distributed database platform to provide \emph{big data management and analytics as a service} primarily for scientific applications.  Myria takes queries in high level query language and them translate them into physical query plan, which will be executed in parallel. After the submitting a query, the two only things that an user can possibly get are the query result and the running time. In practice an user would like to ask questions like why this query is slow, why the result of this query is so few, etc. These questions are challenging and difficult for Myria  other current big data system to answer.  However, with our visualization system, Myria users can interactively explore the visualization of query processing to find the answers to their questions.

Notably, the abstraction level of Myria that our visualization system interact with is the physical plan layer. Most current distributed database systems supporting structured or semi-structured data model, like Pig, Hive and Shark \cite{pig, hive, XinRZFSS13SIGMOD}, shared the similar design of this layer. Thus, our visualization approach is general to relational big data systems.
...
Myria transforms user's query in the languages that it supports, eg. SQL, Datalog, or MyriaL\footnote{MyriaL is a PigLatin like language} into a physical query plan. Physical query plan can be think of the "execution map" of a query. Usually it can be represented as a \emph{Directed acyclic graph} (DAG), which consists of basic operators, e.g. \emph{JOIN}, \emph{GROUP BY}, \emph{SCAN}, \emph{APPLY} as vertices and the data flows between different operators as edges. Besides the operators that usually exist in sequential relational database system, Myria also has a \emph{SHUFFLE} operator to route the tuples in a sharded table to workers according the user specified column(s) to be hashed. 
...
When a query is submitted by user, the front end translates this query into physical query plan using its query optimizer. Then it submits the physical query plan to master through REST server. The coordinator gets the physical plan from REST sever and begin to call MyriaX to execute the physical plan.

For example, the SQL query $SELECT \; R.*, S.*  \; WHERE \; R.x=S.y ;$ can be translated to the visualized query plan shown in Figure~\ref{fig:query_plan}. The semantics of the query plan is a distributed hash join of two tables, $R$ and $S$. We first need to hash each table according to the joined fields. Then tuples are shuffled by their hash results to different workers. At each worker, the shuffled tuples are joined locally. A query fragment is a subtree of the query execution plan that is separated from the rest of the plan by a network boundary. The example has three query fragments denoted by different colors.
...
Figure~\ref{fig:vis_arch} shows the architecture of our visualization system. We log relevant events in each worker. After the execution of a query, logs stored in workers are collected by the log coordinator on the maser node and then aggregated and  stored to the log database. When there is a request of visualization data from the web UI, the visualization REST server will extract the specific data requested from the log DB and transform it to the JSON format and send it to web UI.  The aim of this design is to have the minimal affect on the query performance yet make the visualization system highly modular for future improvement. For example, using the REST api, anyone can implement their own visualization front end to meet special need.
