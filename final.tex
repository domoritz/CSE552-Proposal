...
In particular, our visualization system is built on top of Myria, a distributed big data management system currently being developed in the UW CSE database group. Myria aims towards building a distributed database platform to provide \emph{big data management and analytics as a service} primarily for scientific applications.  Myria takes queries in high level query language and then translate then into physical query plan, which will be executed in parallel. After the submitting a query, the two only things that an user can possibly get are the query result and the running time. In practice an user would like to ask questions like why this query is slow, why the result of this query is so few, etc. These questions are challenging and difficult for Myria  other current big data system to answer.  However, with our visualization system, Myria users can interactively explore the visualization of query processing to find the answers to their questions.
